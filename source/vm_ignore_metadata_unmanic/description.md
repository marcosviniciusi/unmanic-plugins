...
**Completely ignores** files that have already completed the full processing pipeline by checking for the `UNMANIC_FULL_PIPELINE_V2=processed` metadata tag.
...
3. This plugin runs **last** (priority 0) and checks for the `UNMANIC_FULL_PIPELINE_V2=processed` tag
4. If the tag exists, it sets `add_file_to_pending_tasks = False` — **overriding all previous plugins**

### Works with

This plugin works together with **Tag Pipeline Complete** (`vm_tag_pipeline_complete`), which writes the `UNMANIC_FULL_PIPELINE_V2=processed` tag as the last processing step. Once a file passes through the entire pipeline, it gets tagged and will never re-enter the queue.
